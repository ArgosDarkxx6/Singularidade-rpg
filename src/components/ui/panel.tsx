...
export function Panel({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('surface-panel min-w-0 rounded-lg border border-white/10 bg-white/[0.03]', className)} {...props} />;
}

export function UtilityPanel({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('min-w-0 rounded-lg border border-white/8 bg-slate-950/55 shadow-[0_14px_30px_rgba(2,8,18,0.28)]', className)} {...props} />;
}
