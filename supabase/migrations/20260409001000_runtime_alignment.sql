...
alter table public.tables
  add column if not exists last_editor text not null default '';
  ...
  select
    ...
    coalesce(
      (
        select jsonb_agg(
          jsonb_build_object(
            'id', character_entry.value ->> 'id',
            'name', character_entry.value ->> 'name',
            'grade', coalesce(character_entry.value ->> 'grade', ''),
            'clan', coalesce(character_entry.value ->> 'clan', '')
          )
          order by character_entry.ordinality
        )
        from jsonb_array_elements(coalesce(t.state -> 'characters', '[]'::jsonb)) with ordinality as character_entry(value, ordinality)
      ),
      '[]'::jsonb
    )
  from public.tables t
